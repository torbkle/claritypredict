...
def generate_synthetic_data(n_samples=100, seed=42):
    ...
    # Nye biomarkører (Gentian-relevante)
    ntprobnp = np.clip(np.random.lognormal(mean=6, sigma=0.5, size=n_samples), 50, 5000)  # pg/mL
    calprotectin = np.clip(np.random.normal(loc=50, scale=30, size=n_samples), 10, 600)   # µg/mL

    # Diagnoser basert på biomarkører
    diagnosis = np.where(
        crp > 20, "Infection",
    ...
    df = pd.DataFrame({
        ...
        "Calprotectin": np.round(calprotectin, 1),
        "Diagnosis": diagnosis
    })
